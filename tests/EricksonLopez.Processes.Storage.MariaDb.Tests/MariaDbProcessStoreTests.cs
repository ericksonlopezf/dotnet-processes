...
namespace EricksonLopez.Processes.Storage.MariaDb.Tests;
...
internal sealed partial class MariaDbTestJsonContext : JsonSerializerContext
{
}

[Trait("Category", "Integration")]
public sealed class MariaDbProcessStoreTests : IClassFixture<MariaDbFixture>
{
    ...
    public MariaDbProcessStoreTests(MariaDbFixture fixture)
    {
        ArgumentNullException.ThrowIfNull(fixture);
        _fixture = fixture;
        _serializer = new SystemTextJsonProcessStateSerializer<SampleOrderState>(
            MariaDbTestJsonContext.Default.SampleOrderState);
    ...
    public async Task SaveAsync_InitialInsert_ShouldPersistInMariaDbAndReturnSuccess()
    {
        ...
        var state = new SampleOrderState("cust-mariadb-1", 149.99m, false);
    ...
    public async Task GetByCorrelationIdAsync_ShouldRetrieveMatchingInstanceFromMariaDb()
    {
        ...
        var state = new SampleOrderState("cust-mariadb-lookup", 89.50m, false);
        var now = DateTimeOffset.UtcNow;
    ...
    public async Task SaveAsync_SequentialAdvances_ShouldUpdateRevisionAndStateInMariaDb()
    {
        ...
        var initial = ProcessInstance<SampleOrderState>.Create(
            ...
            ProcessVersion.Initial,
            correlationId,
            new SampleOrderState("cust-mariadb-2", 60m, false),
            initialTime);
        ...
        var advanced2 = advanced1.Advance(
            new SampleOrderState("cust-mariadb-2", 60m, true),
    ...
    public async Task SaveAsync_ConcurrentStaleRevision_ShouldReturnConcurrencyConflictInMariaDb()
    {
        ...
        var advanced1 = initial.Advance(
            new SampleOrderState("cust-mariadb-3", 250m, false),
            ProcessStatus.Running,
            DateTimeOffset.UtcNow);
        var result1 = await _store.SaveAsync(advanced1);
        result1.Should().Be(ProcessSaveResult.Success);
        ...
        var staleAdvance = initial.Advance(
            new SampleOrderState("cust-mariadb-3", 300m, false),
            ProcessStatus.Running,
            DateTimeOffset.UtcNow);
    ...
    public async Task SaveAsync_WithComplexNestedState_ShouldPersistAndRetrieveCorrectly()
    {
        var complexSerializer = new SystemTextJsonProcessStateSerializer<ComplexOrderState>(
            MariaDbTestJsonContext.Default.ComplexOrderState);
        ...
        var state = new ComplexOrderState(
            ...
            new List<OrderItem>
            {
                new OrderItem("SKU-MARIADB-1", 2, 99.99m),
                new OrderItem("SKU-MARIADB-2", 1, 149.50m)
            });
        ...
        loaded.State.Items[1].Sku.Should().Be("SKU-MARIADB-2");
    }

    [Fact]
    public void Constructor_InvalidArguments_ShouldThrowExpectedExceptions()
    {
        var actNullConn = () => new MariaDbProcessStore<SampleOrderState>(null!, _serializer);
        actNullConn.Should().ThrowExactly<ArgumentNullException>().WithParameterName("connectionString");

        var actEmptyConn = () => new MariaDbProcessStore<SampleOrderState>("   ", _serializer);
        actEmptyConn.Should().ThrowExactly<ArgumentException>().WithParameterName("connectionString");

        var actNullTable = () => new MariaDbProcessStore<SampleOrderState>(_fixture.ConnectionString, _serializer, tableName: null!);
        actNullTable.Should().ThrowExactly<ArgumentNullException>().WithParameterName("tableName");

        var actEmptyTable = () => new MariaDbProcessStore<SampleOrderState>(_fixture.ConnectionString, _serializer, tableName: "  ");
        actEmptyTable.Should().ThrowExactly<ArgumentException>().WithParameterName("tableName");

        var actInvalidTable = () => new MariaDbProcessStore<SampleOrderState>(_fixture.ConnectionString, _serializer, tableName: "123_invalid!");
        ...
        var actNullSerializer = () => new MariaDbProcessStore<SampleOrderState>(_fixture.ConnectionString, null!);
        actNullSerializer.Should().ThrowExactly<ArgumentNullException>().WithParameterName("serializer");

        var customLogger = NullLogger<MariaDbProcessStore<SampleOrderState>>.Instance;
        var storeWithLogger = new MariaDbProcessStore<SampleOrderState>(_fixture.ConnectionString, _serializer, "process_instances", customLogger);
    ...
    {
        var id = ProcessId.NewId();
        var correlationId = CorrelationId.From("corr-mariadb-dup-initial");
        var instance = ProcessInstance<SampleOrderState>.Create(
            id,
            ProcessType.From("order.mariadb.dup"),
            ProcessVersion.Initial,
            correlationId,
            new SampleOrderState("cust-mariadb-dup", 50m, false),
            DateTimeOffset.UtcNow);
    ...
    {
        var nonExistentCorr = CorrelationId.From($"corr-mariadb-none-{Guid.NewGuid():N}");
    ...
    {
        const string customTable = "custom_mariadb_process_instances";
        ...
        var customStore = new MariaDbProcessStore<SampleOrderState>(_fixture.ConnectionString, _serializer, tableName: customTable);
        var id = ProcessId.NewId();
        var corr = CorrelationId.From("corr-mariadb-custom-table");
        var instance = ProcessInstance<SampleOrderState>.Create(
            id,
            ProcessType.From("order.mariadb.custom"),
            ProcessVersion.Initial,
            corr,
            new SampleOrderState("cust-mariadb-custom", 99m, false),
            DateTimeOffset.UtcNow);
    ...
    [Fact]
    public void AddMariaDbProcessStore_DI_Registration_ShouldResolveStoreSuccessfully()
    {
        var services = new ServiceCollection();
        services.AddSingleton<IProcessStateSerializer<SampleOrderState>>(_serializer);
        services.AddSingleton(typeof(ILogger<>), typeof(NullLogger<>));

        services.AddMariaDbProcessStore<SampleOrderState>(_fixture.ConnectionString, "process_instances");

        var provider = services.BuildServiceProvider();
        var resolvedStore = provider.GetService<IProcessStore<SampleOrderState>>();

        resolvedStore.Should().NotBeNull();
        resolvedStore.Should().BeOfType<MariaDbProcessStore<SampleOrderState>>();
    }

    [Fact]
    public void AddMariaDbProcessStore_InvalidArguments_ShouldThrowExpectedExceptions()
    {
        var services = new ServiceCollection();

        var actNullServices = () => ProcessMariaDbServiceCollectionExtensions.AddMariaDbProcessStore<SampleOrderState>(null!, _fixture.ConnectionString);
        actNullServices.Should().Throw<ArgumentNullException>().WithParameterName("services");

        var actNullConn = () => services.AddMariaDbProcessStore<SampleOrderState>(null!);
        actNullConn.Should().Throw<ArgumentException>().WithParameterName("connectionString");

        var actEmptyConn = () => services.AddMariaDbProcessStore<SampleOrderState>("   ");
