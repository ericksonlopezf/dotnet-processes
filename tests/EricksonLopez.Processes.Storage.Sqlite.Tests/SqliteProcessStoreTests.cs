...
internal sealed partial class SqliteTestJsonContext : JsonSerializerContext
{
}

public sealed class SqliteProcessStoreTests : IClassFixture<SqliteFixture>
{
    ...
    public SqliteProcessStoreTests(SqliteFixture fixture)
    {
        ArgumentNullException.ThrowIfNull(fixture);
        _fixture = fixture;
        _serializer = new SystemTextJsonProcessStateSerializer<SampleOrderState>(
            SqliteTestJsonContext.Default.SampleOrderState);
    ...
    public async Task SaveAsync_SequentialAdvances_ShouldUpdateRevisionAndStateInSqlite()
    {
        ...
        finalLoaded.CreatedAt.Should().Be(initialTime);
        finalLoaded.UpdatedAt.Should().Be(completedTime);
        finalLoaded.CompletedAt.Should().Be(completedTime);

        var loadedByCorr = await _store.GetByCorrelationIdAsync(correlationId);
        loadedByCorr.Should().NotBeNull();
        loadedByCorr!.CompletedAt.Should().Be(completedTime);
    }
    ...
    public async Task SaveAsync_WithComplexNestedState_ShouldPersistAndRetrieveCorrectly()
    {
        var complexSerializer = new SystemTextJsonProcessStateSerializer<ComplexOrderState>(
            SqliteTestJsonContext.Default.ComplexOrderState);
    ...
    public void Constructor_InvalidArguments_ShouldThrowExpectedExceptions()
    {
        ...
        var actInvalidTable = () => new SqliteProcessStore<SampleOrderState>(_fixture.ConnectionString, _serializer, tableName: "123_invalid!");
        ...
        var customLogger = NullLogger<SqliteProcessStore<SampleOrderState>>.Instance;
        var storeWithLogger = new SqliteProcessStore<SampleOrderState>(_fixture.ConnectionString, _serializer, "ProcessInstances", customLogger);
    ...
    public async Task SaveAsync_WithCustomTableName_ShouldPersistAndQueryCorrectly()
    {
        ...
        await using (var conn = new SqliteConnection(_fixture.ConnectionString))
        {
            ...
            const string ddl = $"""
                ...
                """;
            await using var cmd = new SqliteCommand(ddl, conn);
