...
namespace EricksonLopez.Processes.Storage.MySql.Tests;
...
internal sealed partial class MySqlTestJsonContext : JsonSerializerContext
{
}

[Trait("Category", "Integration")]
public sealed class MySqlProcessStoreTests : IClassFixture<MySqlFixture>
{
    ...
    public MySqlProcessStoreTests(MySqlFixture fixture)
    {
        ArgumentNullException.ThrowIfNull(fixture);
        _fixture = fixture;
        _serializer = new SystemTextJsonProcessStateSerializer<SampleOrderState>(
            MySqlTestJsonContext.Default.SampleOrderState);
    ...
    public async Task SaveAsync_InitialInsert_ShouldPersistInMySqlAndReturnSuccess()
    {
        ...
        var state = new SampleOrderState("cust-mysql-1", 149.99m, false);
    ...
    public async Task GetByCorrelationIdAsync_ShouldRetrieveMatchingInstanceFromMySql()
    {
        ...
        var state = new SampleOrderState("cust-mysql-lookup", 89.50m, false);
        var now = DateTimeOffset.UtcNow;
    ...
    public async Task SaveAsync_SequentialAdvances_ShouldUpdateRevisionAndStateInMySql()
    {
        ...
        var initial = ProcessInstance<SampleOrderState>.Create(
            ...
            ProcessVersion.Initial,
            correlationId,
            new SampleOrderState("cust-mysql-2", 60m, false),
            initialTime);
        ...
        var advanced2 = advanced1.Advance(
            new SampleOrderState("cust-mysql-2", 60m, true),
    ...
    public async Task SaveAsync_ConcurrentStaleRevision_ShouldReturnConcurrencyConflictInMySql()
    {
        ...
        var advanced1 = initial.Advance(
            new SampleOrderState("cust-mysql-3", 250m, false),
            ProcessStatus.Running,
            DateTimeOffset.UtcNow);
        var result1 = await _store.SaveAsync(advanced1);
        result1.Should().Be(ProcessSaveResult.Success);
        ...
        var staleAdvance = initial.Advance(
            new SampleOrderState("cust-mysql-3", 300m, false),
            ProcessStatus.Running,
            DateTimeOffset.UtcNow);
    ...
    public async Task SaveAsync_WithComplexNestedState_ShouldPersistAndRetrieveCorrectly()
    {
        var complexSerializer = new SystemTextJsonProcessStateSerializer<ComplexOrderState>(
            MySqlTestJsonContext.Default.ComplexOrderState);
        ...
        var state = new ComplexOrderState(
            ...
            new List<OrderItem>
            {
                new OrderItem("SKU-MYSQL-1", 2, 99.99m),
                new OrderItem("SKU-MYSQL-2", 1, 149.50m)
            });
        ...
        loaded.State.Items[1].Sku.Should().Be("SKU-MYSQL-2");
    }

    [Fact]
    public void Constructor_InvalidArguments_ShouldThrowExpectedExceptions()
    {
        var actNullConn = () => new MySqlProcessStore<SampleOrderState>(null!, _serializer);
        actNullConn.Should().ThrowExactly<ArgumentNullException>().WithParameterName("connectionString");

        var actEmptyConn = () => new MySqlProcessStore<SampleOrderState>("   ", _serializer);
        actEmptyConn.Should().ThrowExactly<ArgumentException>().WithParameterName("connectionString");

        var actNullTable = () => new MySqlProcessStore<SampleOrderState>(_fixture.ConnectionString, _serializer, tableName: null!);
        actNullTable.Should().ThrowExactly<ArgumentNullException>().WithParameterName("tableName");

        var actEmptyTable = () => new MySqlProcessStore<SampleOrderState>(_fixture.ConnectionString, _serializer, tableName: "  ");
        actEmptyTable.Should().ThrowExactly<ArgumentException>().WithParameterName("tableName");

        var actInvalidTable = () => new MySqlProcessStore<SampleOrderState>(_fixture.ConnectionString, _serializer, tableName: "123_invalid!");
        ...
        var actNullSerializer = () => new MySqlProcessStore<SampleOrderState>(_fixture.ConnectionString, null!);
        actNullSerializer.Should().ThrowExactly<ArgumentNullException>().WithParameterName("serializer");

        var customLogger = NullLogger<MySqlProcessStore<SampleOrderState>>.Instance;
        var storeWithLogger = new MySqlProcessStore<SampleOrderState>(_fixture.ConnectionString, _serializer, "process_instances", customLogger);
    ...
    {
        var id = ProcessId.NewId();
        var correlationId = CorrelationId.From("corr-mysql-dup-initial");
        var instance = ProcessInstance<SampleOrderState>.Create(
            id,
            ProcessType.From("order.mysql.dup"),
            ProcessVersion.Initial,
            correlationId,
            new SampleOrderState("cust-mysql-dup", 50m, false),
            DateTimeOffset.UtcNow);
    ...
    {
        var nonExistentCorr = CorrelationId.From($"corr-mysql-none-{Guid.NewGuid():N}");
    ...
    {
        const string customTable = "custom_mysql_process_instances";
        ...
        var customStore = new MySqlProcessStore<SampleOrderState>(_fixture.ConnectionString, _serializer, tableName: customTable);
        var id = ProcessId.NewId();
        var corr = CorrelationId.From("corr-mysql-custom-table");
        var instance = ProcessInstance<SampleOrderState>.Create(
            id,
            ProcessType.From("order.mysql.custom"),
            ProcessVersion.Initial,
            corr,
            new SampleOrderState("cust-mysql-custom", 99m, false),
            DateTimeOffset.UtcNow);
    ...
    [Fact]
    public void AddMySqlProcessStore_DI_Registration_ShouldResolveStoreSuccessfully()
    {
        var services = new ServiceCollection();
        services.AddSingleton<IProcessStateSerializer<SampleOrderState>>(_serializer);
        services.AddSingleton(typeof(ILogger<>), typeof(NullLogger<>));

        services.AddMySqlProcessStore<SampleOrderState>(_fixture.ConnectionString, "process_instances");

        var provider = services.BuildServiceProvider();
        var resolvedStore = provider.GetService<IProcessStore<SampleOrderState>>();

        resolvedStore.Should().NotBeNull();
        resolvedStore.Should().BeOfType<MySqlProcessStore<SampleOrderState>>();
    }

    [Fact]
    public void AddMySqlProcessStore_InvalidArguments_ShouldThrowExpectedExceptions()
    {
        var services = new ServiceCollection();

        var actNullServices = () => ProcessMySqlServiceCollectionExtensions.AddMySqlProcessStore<SampleOrderState>(null!, _fixture.ConnectionString);
        actNullServices.Should().Throw<ArgumentNullException>().WithParameterName("services");

        var actNullConn = () => services.AddMySqlProcessStore<SampleOrderState>(null!);
        actNullConn.Should().Throw<ArgumentException>().WithParameterName("connectionString");

        var actEmptyConn = () => services.AddMySqlProcessStore<SampleOrderState>("   ");
