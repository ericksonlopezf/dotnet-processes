...
/// <summary>
/// Provides a SQLite implementation of <see cref="IProcessStore{TState}"/> using raw parameterized SQLite commands and serialized JSON payloads.
/// </summary>
...
public sealed partial class SqliteProcessStore<TState> : IProcessStore<TState>
    where TState : notnull
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="SqliteProcessStore{TState}"/> class with the specified connection string and serializer.
    /// </summary>
...
#pragma warning restore CA2100, S2077
        command.Parameters.AddWithValue("$ProcessId", id.Value.ToString());
...
#pragma warning disable CA2100, S2077
        ...
        await using var command = new SqliteCommand(sql, connection);
#pragma warning restore CA2100, S2077
        command.Parameters.AddWithValue("$CorrelationId", correlationId.Value);
    ...
    {
#pragma warning disable CA2100, S2077
        var sql = $"SELECT 1 FROM \"{_tableName}\" WHERE ProcessId = $ProcessId LIMIT 1;";
...
#pragma warning restore CA2100, S2077
        ...
        updateCmd.Parameters.AddWithValue("$ExpectedRevision", expectedPreviousRevision);
    ...
    private static void AddParameters(SqliteCommand cmd, ProcessInstance<TState> instance, string payloadJson)
    {
        cmd.Parameters.AddWithValue("$ProcessId", instance.Id.Value.ToString());
        cmd.Parameters.AddWithValue("$ProcessType", instance.Type.Value);
        cmd.Parameters.AddWithValue("$Version", instance.Version.Value);
        cmd.Parameters.AddWithValue("$Status", (int)instance.Status);
        cmd.Parameters.AddWithValue("$Revision", instance.Revision.Value);
        cmd.Parameters.AddWithValue("$CorrelationId", instance.CorrelationId.Value);
        cmd.Parameters.AddWithValue("$StatePayload", payloadJson);
        cmd.Parameters.AddWithValue("$CreatedAt", instance.CreatedAt.ToString("o", CultureInfo.InvariantCulture));
        cmd.Parameters.AddWithValue("$UpdatedAt", instance.UpdatedAt.ToString("o", CultureInfo.InvariantCulture));
        if (instance.CompletedAt.HasValue)
        {
            cmd.Parameters.AddWithValue("$CompletedAt", instance.CompletedAt.Value.ToString("o", CultureInfo.InvariantCulture));
        }
        else
        {
    ...
    private ProcessInstance<TState> MapFromReader(SqliteDataReader reader)
    {
