...
namespace EricksonLopez.Processes.Storage.MySql;

/// <summary>
/// Provides extension methods for registering MySQL process store persistence with an <see cref="IServiceCollection"/>.
/// </summary>
public static class ProcessMySqlServiceCollectionExtensions
{
    /// <summary>
    /// Registers <see cref="MySqlProcessStore{TState}"/> as the implementation of <see cref="IProcessStore{TState}"/> in the service collection.
    /// </summary>
    ...
    public static IServiceCollection AddMySqlProcessStore<TState>(
        ...
        where TState : notnull
    {
        ...
        services.TryAddSingleton<IProcessStore<TState>>(sp =>
        {
            ...
            var logger = sp.GetService<ILogger<MySqlProcessStore<TState>>>();
            return new MySqlProcessStore<TState>(connectionString, serializer, tableName, logger);
        });
